...
namespace TD5
{
    class Etagere : IImprimable
    {
        ...
        public void Imprimer(int noEnregistrement)
        {
            try
            {
                if (listeDocuments.ElementAt(noEnregistrement - 1) != null)
                {
                    if (listeDocuments.ElementAt(noEnregistrement - 1).IsImprimable)
                    {
                        Console.WriteLine("Imprimer...\n" + listeDocuments.ElementAt(noEnregistrement - 1).ToString());
                    }
                    else
                    {
                        Console.WriteLine("Vous ne pouvez pas imprimer ce document !");
                    }
                }
            } catch
            {
