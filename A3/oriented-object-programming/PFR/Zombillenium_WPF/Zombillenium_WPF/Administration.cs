...
namespace Zombillenium_WPF
{
    public class Administration
    {
        ...
        internal List<Attraction> Attractions { get => attractions; set => attractions = value; }
        internal List<Personnel> ToutLePersonnel { get => toutLePersonnel; set => toutLePersonnel = value; }
        ...
        public bool SendToBAP(Monstre monstre)
        {
            ...
            for (int i = 0; i < attractions.Count(); i++)
            {
                if (attractions.ElementAt(i).GetType().Equals(typeof(Boutique)))
                {
                    ...
                    if (b.TypeBoutique == TypeBoutique.barbeAPapa)
                    {
                        barbeAPapas.Add(b);
                    }
                }
        ...
        public bool ModifierCagnotte(int matricule, int newCagnotte)
        {
            Personnel employe = DoesEmployeExists(matricule);
            if (employe != null && !(employe is Sorcier) && newCagnotte >= 0)
            {
                Monstre monster = employe as Monstre;
                monster.Cagnotte = newCagnotte;
        ...
        public void ModifierMaintenance(int id, string natureMaintenance, double dureeMaintenanceInDays)
        {
            ...
            {
                if (natureMaintenance.Equals("") || dureeMaintenanceInDays == 0.0) // plus de maintenance
                {
